    ...
    @foreach($testimonials as $i => $t)
        <div x-show="isActive({{ $i }})" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="text-center px-4 py-4 md:px-8 md:py-6">
            ...
            <div class="flex items-center justify-center gap-3">
                @if(!empty($t['photo']))
                    <img src="{{ (str_starts_with($t['photo'], 'http') ? $t['photo'] : \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($t['photo'])) }}" alt="{{ $t['author'] ?? '' }}" class="w-10 h-10 rounded-full object-cover" />
                @endif
                <div class="text-left">
