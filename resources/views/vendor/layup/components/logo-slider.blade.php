    ...
    <div class="flex" style="animation: layup-logo-slide {{ $speed }}s linear infinite; width: max-content">
        @for($i = 0; $i < 2; $i++)
            @foreach($logos as $logo)
                ...
                @endphp
                @if(!empty($logoSrc))
                    <img src="{{ str_starts_with($logoSrc, 'http') ? $logoSrc : \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($logoSrc) }}" alt="{{ $logoAlt }}" style="max-height: {{ $maxH }}; width: auto; margin-right: {{ $gap }}" class="shrink-0 opacity-70 hover:opacity-100 transition-opacity" />
                @endif
            @endforeach
        @endfor
    </div>
