...
>
    @foreach(($data['images'] ?? []) as $image)
        @php
            ...
            $imgUrl = (!empty($imgSrc) && str_starts_with($imgSrc, 'http')) ? $imgSrc : (!empty($imgSrc) ? \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($imgSrc) : '');
        @endphp
        @if(!empty($imgSrc))
