...
>
    @foreach(($data['members'] ?? []) as $member)
        <div class="text-center">
            ...
            @if(!empty($photoSrc))
                <img src="{{ str_starts_with($photoSrc, 'http') ? $photoSrc : \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($photoSrc) }}" alt="{{ $member['name'] ?? '' }}" class="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover mx-auto mb-3" />
            @else
