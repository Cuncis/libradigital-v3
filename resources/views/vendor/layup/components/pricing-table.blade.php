<div @if(!empty($data['id']))id="{{ $data['id'] }}"@endif class="border rounded-xl p-4 md:p-6{{ !empty($data['featured']) ? 'layup-border-primary ring-2 ring-[var(--layup-primary)] relative' : 'border-gray-200 dark:border-gray-700' }} {{ \Crumbls\Layup\View\BaseView::visibilityClasses($data['hide_on'] ?? []) }} {{ $data['class'] ?? '' }}" style="{{ \App\Layup\Support\StyleHelper::buildInlineStyles($data) }}" {!! \Crumbls\Layup\View\BaseView::animationAttributes($data) !!}>
