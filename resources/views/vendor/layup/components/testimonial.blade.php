<div @if(!empty($data['id']))id="{{ $data['id'] }}"@endif class="border-l-4 pl-4 md:pl-6 py-2{{ \Crumbls\Layup\View\BaseView::visibilityClasses($data['hide_on'] ?? []) }} {{ $data['class'] ?? '' }}" style="border-color: var(--layup-primary); {{ \App\Layup\Support\StyleHelper::buildInlineStyles($data) }}" {!! \Crumbls\Layup\View\BaseView::animationAttributes($data) !!}>
