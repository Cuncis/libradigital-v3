    ...
    <table class="w-full text-sm border-collapse">
        <thead>
            <tr>
                ...
                <th class="text-center p-3 border-b-2 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400">{{ $data['column_b'] ?? 'Them' }}</th></tr></thead>
        <tbody>
            @foreach(($data['rows'] ?? []) as $i => $row)
                <tr class="{{ $i % 2 === 0 ? 'bg-gray-50 dark:bg-gray-800' : '' }}">
                    ...
                    <td class="p-3 border-b border-gray-100 dark:border-gray-800 text-center text-gray-400 dark:text-gray-500">{{ $row['value_b'] ?? '' }}</td></tr>
            @endforeach
        </tbody></table></div>
