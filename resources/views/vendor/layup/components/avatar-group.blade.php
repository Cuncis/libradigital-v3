...
>
    <div class="flex -space-x-2">
        @foreach(($data['avatars'] ?? []) as $avatar)
            ...
            @endphp
            @if(!empty($src))
                <img src="{{ str_starts_with($src, 'http') ? $src : \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($src) }}" alt="{{ $alt }}" class="{{ $size }} rounded-full border-2 border-white dark:border-gray-900 object-cover" />
            @endif
        @endforeach
