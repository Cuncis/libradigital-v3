...
>
    ...
    <div id="{{ $lgId }}">
        @foreach(($data['logos'] ?? []) as $logo)
            ...
            @endphp
            @if(!empty($logoSrc))
                <img src="{{ str_starts_with($logoSrc, 'http') ? $logoSrc : \Illuminate\Support\Facades\Storage::disk(config('layup.uploads.disk', 'public'))->url($logoSrc) }}" alt="{{ $logoAlt }}" style="max-height:{{ $maxH }};width:auto" class="{{ $gray ? 'grayscale hover:grayscale-0 opacity-60 hover:opacity-100' : '' }} transition-all duration-300" />
            @endif
        @endforeach
    </div></div>
