...
## Ad-hoc: row/column/widget settings open from the left, Elementor-style

In Elementor, clicking a section, column, or widget shows its settings in the same left panel the widget library normally occupies (sliding over it), not a separate right-hand drawer. Layup's row/column/widget edit actions already used Filament's `->slideOver()` — they just opened from the default (right) edge.

Investigated first rather than assuming a rebuild was needed: reimplementing Layup's edit forms as a hand-rolled Alpine panel (to literally swap the widget list's own DOM for an edit form) would mean re-implementing Filament's schema rendering for every field type those forms use (Select, ColorPicker, the custom SpacingPicker, etc.) — a large, fragile undertaking for a purely cosmetic difference from what Filament's own modal system already provides. `Filament\Support\Enums\SlideOverPosition::Start` gets the same visual result — the panel covers the same screen region, same slide direction, same "editing this thing" affordance — using Filament's own modal system entirely as-is.

`App\Layup\Forms\Components\LayupBuilder::rowEditAction()`/`columnEditAction()`/`widgetEditAction()` now override Layup's versions, calling `parent::xxxAction()` and chaining `->slideOverPosition(SlideOverPosition::Start)->modalWidth(Width::Small)` — same actions, same forms, same everything Layup already built, just anchored to the other edge and narrowed to roughly match the sidebar it's covering. Picked up automatically with no other wiring: the base field's `setUp()` registers actions via `$this->rowEditAction()` (late static binding), so overriding the method in the subclass is all that's needed. Extended to widget editing too, not just row/column as literally asked, since having some panels slide from the left and others from the right would read as inconsistent rather than deliberate — flagged in case that's not wanted.

**Test**: `LayupBuilderEditActionsTest` — all three actions assert `isModalSlideOver()`, `getModalSlideOverPosition() === Start`, `getModalWidth() === Small`, checked directly on the constructed Action objects (works without any form/Livewire container context — these are just configuration values). Full suite: 150/150 passing.

## Ad-hoc: full-screen editor page, Elementor-Pro-style

Edit Invitation is now a genuine full-screen takeover — no sidebar, no topbar — the same way Elementor Pro's editor replaces wp-admin's chrome entirely instead of living inside it, rather than just being a full-width page still wrapped in the dashboard shell.

**How, without adding weight**: Filament pages resolve their outer chrome (sidebar + topbar) and their own content (breadcrumbs, header actions, form) through two *separate* mechanisms — `getLayout()` (the outer `<html>` shell) and the page's own `$view` (rendered inside it). Confirmed by reading `filament-panels::components.layout.index` (sidebar + topbar wrapping a `<main>{{ $slot }}</main>`) versus `<x-filament-panels::page>` (breadcrumbs/header/actions/content — no sidebar or topbar at all): overriding *only* `getLayout()` on `EditInvitation` (both admin and user) to a new `resources/views/filament/layouts/full-screen-editor.blade.php` — which still reuses Filament's own `<x-filament-panels::layout.base>` for the shared `<head>` (CSRF, dark mode, all Livewire/Alpine/Filament assets, notifications) — removes the sidebar/topbar Livewire components entirely while leaving the page's own header (Preview/Save changes/Delete actions, breadcrumbs back to the list) completely untouched. Adds no new CSS/JS of its own — it only *removes* two Livewire components from the render tree, which is the "lightweight" part: nothing new to load, something removed.

**Test note**: first attempt at proving the sidebar was gone asserted the admin sidebar wouldn't show a resource label like "Users" — that assertion failed not because the fix didn't work, but because Filament's global search (Cmd+K) embeds every resource's name into a JS payload loaded on *every* page regardless of whether the sidebar itself renders, so a resource label proves nothing about sidebar presence either way. Switched to asserting on the sidebar/topbar Livewire components' own structural markup (`id="fi-main-sidebar"`, `class="fi-topbar-ctn"`) instead, verified present on the *list* page and absent on the edit page — a baseline that would have caught a broken/empty-everywhere layout the label-based check couldn't.

**The row/col/widget settings panel sizing** (from the previous entry) was already uniform — all three already use `Width::Small`. Checked whether their form fields would reflow awkwardly in that narrower space: Layup's own row/column/widget form schemas don't set an explicit column count anywhere (Filament's schema grids default to 1 column absent an override, and are viewport-breakpoint-based rather than container-width-based, confirmed by reading `HasContainerGridLayout`), so nothing here should be laying out multi-column against a narrow panel. The one multi-column layout in play, `SpacingPicker::advanced()`'s padding/margin box-model UI, is a fixed 3-column grid regardless of container width — vendor code, deliberately compact by design (the same idea as WordPress's own spacing box control). No concrete broken layout to point to from reading the code — flagging this rather than guessing at a CSS fix with nothing broken to fix.
